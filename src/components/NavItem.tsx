                  ...
                  {category.featured.map((product) => (
                    <div key={product.name} className="group relative text-base sm:text-sm">
                      <div className="relative aspect-video overflow-hidden rounded-lg bg-gray-100 group-hover:opacity-75">
                        <Image src={product.imageSrc} alt={product.name} fill className="object-cover object-center" />
                      </div>

                      <Link href={product.href} className="mt-6 block font-medium text-gray-900">
                        {product.name}
                      </Link>
